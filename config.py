...
def setup(ex):
    ...
    def ex_config():
        ...
        ann_type = 'dense'              # dense /bbox /scribble
        ...
        scale_min = 0.9
        scale_max = 1.1
        dataset = "iSAID"              # str, dataset name. [PASCAL|COCO]
        use_split_coco = True
        rotate = 10
        ...
        test_n = 5000                   # default 5000; int, number of test examples in each run
        ...
        pascal2coco = False             # bool, flag for evaluating in domain shift scenario of pascal -> coco
        #默认lr-0.002，optim SGD
        # Training configurations  ============================================================
        epochs = 80                     # int, Number of total epochs for training
        lr = 0.001                    #  bs8/0.001  default bs8/0.002 float, Base learning rate for model training
        ...
        if lrp == "period_step":
            lr_step = 24          # int, [period_step] Decay the base learning rate at a fixed step
        ...
        if lrp == 'cosinev2':
            lr_repeat = 2
            lr_rev = False
        optim = "sgd"                   # str, Optimizer for training [sgd/adam/adamw|sam]
        if optim == "adam":
        ...
        precompute_weight = False        # bool, precompute weights for accelerating training (2x)
        pair_lossW = 0.02               # float, loss weight for pairwise loss
        contract_lossW = 0.01               # float, loss weight for contract_loss

        # Network configurations ====================================================================
        network = "agmtr"             # str, network name
        ...
        unlabeled = 0                   # int, number of unlabeled images
        agg_depth = 1                   # int, number of ASD
        filter = 0.7
        fg_num = 5                      # int, number of Agents
    ...
    # ex.add_config('configs/pascal_vit.yml')
    ex.add_config('configs/pascal_deit.yml')
    # ex.add_config('configs/coco_vit.yml')
    # ex.add_config('configs/coco_deit.yml')
    # ex.add_config('configs/iSAID_vit.yml')
    # ex.add_config('configs/iSAID_deit.yml')
