...
class FewShotMetric_GFSS(object):
    def __init__(self, n_class):
        self.n_class = n_class
        self.stat = np.zeros((self.n_class + 1, 3))  # +1 for bg, 3 for tp, fp, fn

    def update(self, pred, ref, cls, ori_size=None, verbose=0): # cls:base+1=16/61
        pred = np.asarray(pred, np.uint8)
        ref = np.asarray(ref, np.uint8)
        for i, ci in enumerate(cls):  # iter on batch ==> [episode_1, episode_2, ...]
            p = pred[i]
            ...
            if ori_size is not None:
                ...
                p = p[:, :ori_H, :ori_W]
                r = r[:, :ori_H, :ori_W]
            ci  = torch.cat((torch.zeros(1),ci),dim=0)

            for j, c in enumerate(ci):  # iter on class ==> [bg_cls, fg_cls]
                tp = int((np.logical_and(p == j, r != 255) * np.logical_and(r == j, r != 255)).sum())
                print(j,c,np.logical_and(p == j, r != 255).sum(),np.logical_and(r == j, r != 255).sum())
                fp = int((np.logical_and(p == j, r != 255) * np.logical_and(r != j, r != 255)).sum())
                fn = int((np.logical_and(p != j, r != 255) * np.logical_and(r == j, r != 255)).sum())
                if verbose:
                    print(tp / (tp + fp + fn))
                self.stat[int(c), 0] += tp
                self.stat[int(c), 1] += fp
                self.stat[int(c), 2] += fn

    def get_scores(self, val_labels,total_labels,base_labels, binary=False):
        """
        Parameters
        ----------
        labels: list, compute mean iou on which classes
        ...
        if binary:
            ...
            stat = self.stat[total_labels]  # [N, 3]
        val_labels = [i-1 for i in val_labels]
        base_labels = [i-1 for i in base_labels]
        tp, fp, fn = stat.T  # [2 or N]
        mIoU_class = tp / (tp + fp + fn)  # [2 or N]
        mean = np.nanmean(mIoU_class)  # scalar
        novel_mean = np.nanmean(mIoU_class[val_labels])
        base_mean = np.nanmean(mIoU_class[base_labels])

        return mIoU_class, mean,novel_mean,base_mean
