...
DATA_LIST = {
    "PASCAL": {
        ...
        "test_GFSS": lists_dir / "pascal/val.txt",
        "eval_online": lists_dir / "pascal/val.txt"
    },
    "COCO": {
        "train": lists_dir / "coco/train_data_list.txt",
        "test": lists_dir / "coco/val_data_list.txt",
        "test_GFSS": lists_dir / "coco/val_data_list.txt",
        "eval_online": lists_dir / "coco/val_data_list.txt"
    },
    "iSAID": {
        "train": lists_dir / "iSAID/train.txt",
        "test": lists_dir / "iSAID/val.txt",
        "test_GFSS": lists_dir / "iSAID/val.txt",
        "eval_online": lists_dir / "iSAID/val.txt"
    }
}
...
def load(opt, logger, mode):
    ...
    ann_type = opt.ann_type
    if mode == "train":
        data_transform = get_train_transforms(opt, height, width)
    elif mode in ["test", "eval_online", "predict", "test_GFSS"]:
    ...
    if opt.dataset == "PASCAL":
        ...
        num_classes = 80
        cache = False
    elif opt.dataset == "iSAID":
        num_classes = 15
        cache = False
    else:
        raise ValueError(f'Not supported dataset: {opt.dataset}. [PASCAL|COCO|iSAID]')

    if mode == "test_GFSS":
        dataset = pfe.GSemData(opt, split, shot, query, ann_type,
        ...
        dataset = pfe.SemData(opt, split, shot, query,ann_type,
                          data_root=DATA_DIR[opt.dataset],
                          data_list=DATA_LIST[opt.dataset][mode],
                          transform=data_transform,
                          mode=mode,
...
def get_val_labels(opt, mode):
    if opt.dataset == "PASCAL":
        ...
        raise ValueError(f'Only support datasets [PASCAL|COCO|iSAID], got {opt.dataset}')
